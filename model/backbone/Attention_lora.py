...
class MultiheadAttention_LoRA(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, lora_alpha: int = 1, r=0, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None):
        ...
        if self._qkv_same_embed_dim is False:
            ...
            self.in_proj_weight_lora_B = nn.Parameter(torch.empty(2 * embed_dim, r))
        ...
        self.out_proj = Linear_LoRA(embed_dim, embed_dim, bias=True, merge_weights=False, lora_alpha=lora_alpha, r=r)
    ...
    def __setstate__(self, state):
        ...
        super(MultiheadAttention_LoRA, self).__setstate__(state)
...
def multi_head_attention_forward(query: Tensor,
    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            ...
            qkv[..., embed_dim:] += linear(linear(query, in_proj_weight_A), in_proj_weight_B) * in_proj_weight_scaling
