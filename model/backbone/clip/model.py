...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, ffn_adapt_option=None,
                 lora_rank=0):
        super().__init__()
    ...
    def attention(self, x: torch.Tensor, prompt=None):
        ...
        if "prompt" in inspect.signature(self.attn).parameters:
            return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask, prompt=prompt)[0]
        else:
            return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]

    def forward(self, x: torch.Tensor, adapter=None, prompt=None):
        if adapter is not None and not isinstance(adapter, nn.Identity):
            attn_out = adapter(x, add_residual=False) + self.attention(self.ln_1(x), prompt=prompt)
        else:
            attn_out = x + self.attention(self.ln_1(x), prompt=prompt)
        mlp_out = attn_out + self.mlp(self.ln_2(attn_out))
        if adapter is not None and not isinstance(adapter, nn.Identity):
            if self.ffn_adapt_option == 'parallel':
                mlp_out = mlp_out + adapter(attn_out, add_residual=False)
            elif self.ffn_adapt_option == "sequential":
                mlp_out = mlp_out + adapter(mlp_out, add_residual=False)
            else:
                pass
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adapter_list=None, all_prompts=None):
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adapter_list=None, all_prompts=None, ret_all=False):
        ...
        x = self.transformer(x, adapter_list, all_prompts=all_prompts)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, adapter_list=None, ret_all=False):
        return self.visual(image.type(self.dtype), adapter_list, ret_all=ret_all)
